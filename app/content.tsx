...
const HomeContent = ({ shows }: any) => {
	...
	return (
		<main className="container py-4 mt-6">
			...
			{/* Grid Shows */}
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-5 gap-6">{shows && shows.length > 0 && shows.map((show: any, idx: number) => <ShowCard key={idx} {...show} />).slice(0, 8)}</div>
			{watchTrailer && <WatchTrailer url={shows[activeSlider] ? shows[activeSlider].trailer_link : ""} setWatchTrailer={setWatchTrailer} />}
		</main>
	);
};
