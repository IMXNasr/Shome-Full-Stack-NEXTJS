...
const RegisterContent = () => {
	...
	const submitFn = async (e: FormEvent) => {
		...
		if (email && email.includes("@") && username && password && name) {
			setLoading(true);
			const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/auth/register`, { name, username, email, password });
			if (data.error) {
				setLoading(false);
			} else if (data.success) await signIn("credentials", { email, password, redirect: false });
			router.replace("/");
		}
	};
	return (
