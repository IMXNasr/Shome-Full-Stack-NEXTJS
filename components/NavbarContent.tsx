...
const LinkList = ({ mobile = false, onClick }: LinkListProps) => {
	...
	return (
		<ul className={`${!mobile ? "hidden" : ""} xl:flex ${mobile ? "flex flex-col" : "flex-row"} ${!mobile && "items-center"} gap-4`}>
			{/* flex */}
			{navLinks.map((l, idx) => (
				<Link key={idx} onClick={onClick} className={`${params.type && params.type.includes(l.link) && "text-mainColor"} mx-2 hover:text-mainColor transition-colors`} href={l.link}>
					{l.name}
				</Link>
			))}
		</ul>
...
const NavButtons = ({ mobile, onClick, username }: NavButtonsProps) => {
	...
	return (
		<nav>
			<div className="container py-4 flex items-center justify-between">
				...
				<Search />
				{/* Buttons */}
				<NavButtons username={user?.username} />
				{/* Bars */}
